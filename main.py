...
def redraw_game_window():
    ...
    text = font.render(f'Termos = {terms}', 1, WHITE)
...
while run:
    ...
    if terms == 1:
        message = msg.render(">> Setinha pra cima aumenta a quantidade de termos", 1,
                             WHITE)
    elif terms in range(2, 11):
        message = msg.render(">> Isso aí! Continua apertando pra cima campeão", 1, WHITE)
    elif terms in range(11, 31):
        message = msg.render(">> Quanto mais vetores melhor!", 1, WHITE)
    elif terms in range(31, 60):
        message = msg.render(">> Agora foi! Sinal quadrático usando círculos.", 1, WHITE)
    else:
        message = msg.render(">> Dá pra ir pra BAIXO também.", 1, WHITE)
